...
class LevelNumberFilters(YAMLSettings):
    ...
    std_level: Annotated[LevelNumber, Field(
        default=10,
        description='The default log level for all loggers'
    )] = 10

    file_level: Annotated[LevelNumber, Field(
        default=10,
...
class LevelNameOptions(YAMLSettings):
    ...
    uvicorn_level: Annotated[LevelNames, Field(
        default='INFO',
        description='The log level for the uvicorn logger'
    )]

    std_level: Annotated[LevelNames, Field(
        default='DEBUG',
        description='The log level for the standard logger'
    )]

    file_level: Annotated[LevelNames, Field(
        default='WARNING',
        description='The log level for the file logger'
    )]
