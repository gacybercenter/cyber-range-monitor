...
class LogService(DatabaseService[EventLog]):
    ...
    async def get_summary(self) -> LogSummaryResponse:
        ...
        return LogSummaryResponse(totals=totals, previous_logs=prev_logs)
    ...
    async def get_by_level(
        ...
        result = await self.models.db.execute(stmnt)
    ...
    async def level_totals(self) -> EventLogLevelTotals:
        ...
        for levels in EventLogLevel:
            ...
            totals[levels.value.lower()] = await self.models.count_by(stmnt)

        return EventLogLevelTotals(**totals)

    async def most_recent(self, predicate: Any) -> EventLog | None:
    ...
    async def previous_logs(self) -> LastLogs:
        ...
        for levels in (EventLogLevel.CRITICAL, EventLogLevel.ERROR):
            ...
            item = await self.most_recent(EventLog.log_level == levels.value)
            prev_logs[key_name] = item
        return LastLogs(**prev_logs)

    def query_params_to_select(
        self,
