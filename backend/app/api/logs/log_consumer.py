...
class RedisLogConsumer:
    def __init__(
        ...
        *,
        ...
        self._keys = ClientKeys(client_id=client_id)
    ...
    async def get_backlog(self) -> Any:
        return await self._redis.lrange(name=self._keys.backlog, start=0, end=-1)  # type: ignore
    ...
    async def _consumer_loop(self) -> None:
        try:
            ...
            logger.exception(f"An error occurred in the log consumer", exc_info=e)

    async def _to_redis(self, json_str: str) -> None:
        try:
            pipe = self._redis.pipeline()
            pipe.rpush(self._keys.backlog, json_str)
            pipe.ltrim(name=self._keys.backlog, start=-self._max_len, end=-1)
            pipe.publish(channel=self._keys.channel, message=json_str)
            await pipe.execute()
