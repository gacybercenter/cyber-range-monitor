...
class ClientMeta(CustomBaseModel):
    logs_sent: Annotated[int, Field(
        default=0,
        description="The number of logs sent to the client."
    )] = 0
    commands_recieved: Annotated[int, Field(
        default=0,
        description="The number of commands received from the client."
    )] = 0
    errors: Annotated[int, Field(
        default=0,
        description="The number of errors encountered by the client."
    )] = 0


class ClientState(CustomBaseModel):

    connected: Annotated[bool, Field(
        default=False,
        description="Whether the client is connected or not."
    )] = False

    paused: Annotated[bool, Field(
        default=False,
        description="Whether the client is paused or not."
    )] = False

    last_activity: Annotated[float, Field(
    ...
    meta: Annotated[ClientMeta, Field(
        default=ClientMeta(),
        description="The metadata for the client."
    )] = ClientMeta()

    filter: Annotated[LogFilter | None, Field(
        default=None,
        description="The filter for the client."
    )] = None

    def update_meta(self, type: Literal['logs_sent', 'commands_recieved', 'errors']) -> None:
        if type == 'logs_sent':
