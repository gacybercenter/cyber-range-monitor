...
from domains.repository import ModelRepository
...
class DatasourceRepository(ModelRepository[DatasourceDB], Generic[DatasourceDB]):
    ...
    async def create_datasource(self, flattened_obj_in: dict) -> DatasourceDB:
        if "password" not in flattened_obj_in:  # sanity check
            raise HTTPBadRequest("You must provide a password for the datasource")

        flattened_obj_in["password"] = CryptoServices.encrypt(flattened_obj_in["password"])
    ...
    async def update_datasource(
        ...
        if "password" in flattened_obj_in:
            flattened_obj_in["password"] = CryptoServices.encrypt(
    ...
    async def read_password(self, datasource: DatasourceDB) -> str:
        return CryptoServices.decrypt(datasource.password)
