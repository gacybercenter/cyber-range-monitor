...
from app.domains.users.dependency import AdminRequired
...
async def search(
    ...
    """
    stmnt = log_service.query_params_to_stmnt(query_params)
    total = await log_service.models.count_rows_by(stmnt)
    if total == 0:
        raise HTTPNotFound("No logs found matching the query parameters.")

    filter_stmnt = query_params.apply_to_query(stmnt)
    results = await log_service.models.execute_on_all(filter_stmnt)
...
async def logs_from_today(
    ...
    Returns:
        LogQueryResponse -- the logs that match the query parameters
    """
    today_stmnt = log_service.logs_from_today()

    complete_stmnt = log_service.query_params_to_stmnt(
        query_filter,
        today_stmnt
    )
    total = await log_service.models.count_rows_by(complete_stmnt)
    if total == 0:
        raise HTTPNotFound("No logs found matching the query parameters.")
