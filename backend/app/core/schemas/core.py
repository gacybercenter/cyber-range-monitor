...
from typing import (
    Annotated, Any, Generic, Self, TypeVar
)
...
class CustomBaseModel(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        alias_generator=serializers.to_camel,
        json_encoders={
            datetime: serializers.dt_serializer
        }
    )
...
class APIListResponse(CustomBaseModel, Generic[SchemaT]):
    ...
    Arguments:
        Generic {SchemaT} -- the model type of the items in the list
    '''
    total: Annotated[int, Field(
        ..., description="The total number of items in the list"
    )]
    data: Annotated[list[SchemaT] | Any, Field(
...
class APIQueryRequest(StrictModel):
    """base model for all app query parameter request models"""

    skip: Annotated[int, Field(
        default=0,
        ge=0,
        description="The number of records to skip (offset)"
    )] = 0
...
class APIQueryResult(CustomBaseModel):
    ...
    next_skip: Annotated[int, Field(
        ...,
        ge=0,
        description="The number of items to skip for the next 'page' of the query"
    )]
...
class APIQueryResponse(CustomBaseModel, Generic[SchemaT]):
    ...
    def from_results(
        ...
        Arguments:
            db_out {list[Any]} -- the list of models from the database
            query_total {int} -- the total number of items in the database
            taken from the length so it's not re-computed.
            query_params {APIQueryRequest} -- the query params for the request
            to limit the number of results returned

        Returns:
            APIQueryResponse[SchemaT] -- the API response 
        """
