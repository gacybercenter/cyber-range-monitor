...
class HTTPErrorMeta(BaseModel):
    ...
    method: str = Field(..., description="The HTTP method used for the request")
...
class InvalidRequestResponse(BaseModel):
    ...
    def __repr__(self) -> str:
        error_strs = [f"{error['loc'][0]}={error['msg']}" for error in self.errors]
...
class ErrorService:
    ...
    def normalize_pydantic_errors(
        ...
        for error in exc.errors():
            errors.append({"field": error["loc"][0], "error": error["msg"]})
    ...
    async def log_error_meta(self, error_meta: HTTPErrorMeta, db: AsyncSession) -> None:
        if error_meta.status_code >= 500:
            await api_console.critical(json.dumps(error_meta.model_dump()), db)
        elif error_meta.status_code in FLAGGED_STATUS_CODES:
            await api_console.error(
                error_meta.__repr__(), db, f"Status code={error_meta.status_code}"
            )
        else:
            await api_console.warning(error_meta.__repr__(), db)

    async def process_http_error(
...
def register_exc_handlers(app: FastAPI) -> None:
    ...
    async def validation_exc_handler(
        ...
        async with get_session() as session:
            ...
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST, content=err_data.model_dump()
            )
