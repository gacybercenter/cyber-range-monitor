...
class SessionSettings(BaseSettings):
    ...
    '''
    cookie_name: Annotated[str, Field(
        'session_id',
        description='Name of the session cookie stored on the client'
    )]
    cookie_secure: Annotated[bool, Field(
        False, description='Secure flag for the cookie'
    )]
    cookie_http_only: Annotated[bool, Field(
        False, description='HttpOnly flag for the cookie'
    )]
    cookie_samesite: Annotated[SamesiteTypes, Field(
        'lax', description='SameSite flag for the cookie'
    )]
    cookie_expr_hours: Annotated[PositiveInt, Field(
        1, description='Lifetime of the session cookie in hours before it is deleted on the client'
    )]
