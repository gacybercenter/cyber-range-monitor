...
class ApiDocsSettings(BaseSettings):
    ...
    enable: Annotated[bool, Field(True, description="Enable the API documentation")]
...
class ApiConfig(BaseSettings):
    ...
    environment: Annotated[
        str, Field("dev", description="The environment the app is running in")
    ]
    env_file: Annotated[
        str,
        Field(".env", description="The path to the .env file to load the secrets from."),
    ]
    ...
    min_log_level: Annotated[
        str, Field("INFO", description="Minimum event log level to write to the database")
    ]
    use_security_headers: Annotated[
        bool,
        Field(
            False,
            description=(
