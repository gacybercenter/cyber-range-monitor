...
from app.common.errors import HTTPErrorDetails, HTTPErrorResponse, HTTPValidationError
...
async def log_http_error(request: Request, exc: HTTPException) -> None:
    ...
    err_data = HTTPErrorDetails.from_request(
        request=request, status=exc.status_code, exc_details=exc.detail
    )
...
async def request_validation_error_handler(
    ...
    return MsgSpecJSONResponse(
        status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, content=json.model_dump()
    )
