...
class EngineSettings(YAMLSettings):
    ...
    pool_size: Annotated[
        int,
        Field(default=10, description="The number of connections to keep in the pool."),
    ] = 10
...
class DatabaseSettings(YAMLSettings):
    engine: Annotated[
        EngineSettings,
        Field(EngineSettings(), description="The engine settings for sqlalchemy."),
    ]

    file_name: Annotated[
        str, Field(default=":memory:", description="The SQLite database file path.")
    ]
    ...
    run_seed: Annotated[
        bool,
        Field(default=False, description="Whether to run the seed file on startup."),
    ]
