...
from server.utils import httpx_utils
...
async def list_extensions(spec: GuacamoleAPISpec) -> dict:
    path = f'{spec.sessions_url}/extensions'
    return await httpx_utils.fetch_json(path, spec.client)


async def get_self(spec: GuacamoleAPISpec) -> dict:
    path = f'{spec.sessions_url}/self'
    return await httpx_utils.fetch_json(path, spec.client)


def list_users_history(spec: GuacamoleAPISpec) -> AsyncGenerator[bytes]:
    path = f'{spec.history}/users'
    return httpx_utils.fetch_json_stream(spec.client, path)


def list_connections_history(spec: GuacamoleAPISpec) -> AsyncGenerator[bytes]:
    path = f'{spec.history}/connections'
    return httpx_utils.fetch_json_stream(spec.client, path)


async def list_users(spec: GuacamoleAPISpec) -> dict:
    return await httpx_utils.fetch_json(spec.users, spec.client, stream=True)


async def get_user(spec: GuacamoleAPISpec, username: str) -> dict:
    path = f'{spec.users}/{username}'
    return await httpx_utils.fetch_json(path, spec.client)


async def get_user_permissions(spec: GuacamoleAPISpec, username: str) -> dict:
    path = f'{spec.users}/{username}/permissions'
    return await httpx_utils.fetch_json(path, spec.client)


async def get_user_effective_permissions(spec: GuacamoleAPISpec, username: str) -> dict:
    path = f'{spec.users}/{username}/effectivePermissions'
    return await httpx_utils.fetch_json(path, spec.client)


async def get_user_history(spec: GuacamoleAPISpec, username: str) -> dict:
    path = f'{spec.users}/{username}/history'
    return await httpx_utils.fetch_json(path, spec.client)


async def list_usergroups(spec: GuacamoleAPISpec) -> dict:
    return await httpx_utils.fetch_json(spec.usergroups, spec.client, stream=True)


async def get_usergroup(spec: GuacamoleAPISpec, group_name: str) -> dict:
    path = f'{spec.usergroups}/{group_name}'
    return await httpx_utils.fetch_json(path, spec.client)


async def get_usergroup_permissions(spec: GuacamoleAPISpec, group_name: str) -> dict:
    path = f'{spec.usergroups}/{group_name}/permissions'
    return await httpx_utils.fetch_json(path, spec.client)


async def list_connections(spec: GuacamoleAPISpec) -> dict:
...
async def get_connection_history(spec: GuacamoleAPISpec, connection_id: str) -> dict:
    path = f'{spec.connections}/{connection_id}/history'
    return await httpx_utils.fetch_json(path, spec.client, stream=True)


async def get_connection_parameters(spec: GuacamoleAPISpec, connection_id: str) -> dict:
    path = f'{spec.connections}/{connection_id}/parameters'
    return await httpx_utils.fetch_json(path, spec.client)


async def list_sharing_profiles(spec: GuacamoleAPISpec) -> dict:
    return await httpx_utils.fetch_json(spec.sharing_profiles, spec.client, stream=True)


async def get_sharing_profile(spec: GuacamoleAPISpec, profile_id: str) -> dict:
    path = f'{spec.sharing_profiles}/{profile_id}'
    return await httpx_utils.fetch_json(path, spec.client)


async def get_sharing_profile_parameters(spec: GuacamoleAPISpec, profile_id: str) -> dict:
...
async def kill_connections(
    spec: GuacamoleAPISpec, connection_ids: list[str]
) -> dict | Any:
