...
class SqlalchemyConfig(Config):
    ...
    timeout: int = Field(
        default=30, description='The timeout for database connections in seconds.'
    )
    autoflush: bool = Field(
        default=True, description='If True, the session will autoflush before queries.'
    )
    ...
    pool_size: int = Field(
        default=10, description='The size of the database connection pool.'
    )
...
class HttpxConfig(Config):
    ...
    limit_max_connections: int = Field(
        default=50, description='The max number of connections.'
    )
    limit_keepalive_expiry: int = Field(
        default=30, description='The keep-alive expiry time in seconds.'
    )
