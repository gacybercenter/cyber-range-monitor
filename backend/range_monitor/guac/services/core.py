...
from range_monitor.guac.api.dtos import GuacUser
...
class GuacamoleRestService:
    ...
    async def get_summary(self) -> GuacamoleSummary:
        ...
        active_connections = await guac_client.list_active_connections(self.spec)
    ...
    async def get_connection_activity(self) -> ConnectionActivity:
        ...
        user_fetches = (
            guac_client.get_user(self.spec, conn.username)
            for conn in active_connections.values()
        )

        active_users = await asyncio.gather(*user_fetches)
