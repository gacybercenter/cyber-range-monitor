...
class SessionRepository(RedisRepository):
    ...
    async def soft_revoke_all(self, user_id: str) -> None:
        """
        Soft revokes all sessions for a user by updating the credential version in
        Redis.
    ...
    async def verify_cver(self, user_id: str, token_cver: int) -> bool:
        """
        Checks if the credential version in Redis matches the token's credential
        version.
        ...
        return redis_cver is not None and int(redis_cver) == token_cver

    async def save_claim(self, *, jti: str, user_id: str, cver: int, ttl: int) -> bool:
        """
        ...
        pipe.set(jti_key(jti), '1', ex=ttl, nx=True)
    ...
    async def rotate(self, old_jti: str, new_jti: str, remaining_ttl: int) -> bool:
        ...
        bool
            ...
            blacklisted.
        """
        if not await self.redis.set(jti_lock(old_jti), '1', ex=10, nx=True):
            return False

        pipe = self.redis.pipeline()

        pipe.delete(jti_key(old_jti))
        pipe.set(blacklist_key(old_jti), '1', ex=remaining_ttl)
        pipe.set(self.jti_key(new_jti), '1', ex=remaining_ttl, nx=True)
