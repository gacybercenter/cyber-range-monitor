...
class AuthenticationService:
    ...
    def get_token_claim(self, token: str | None, expected_type: str) -> JwtClaim:
        ...
        with jwt_error_codes():
            payload = tokens.decode_jwt_token(
                self.jwt_policy,
                token
            )
            claim = tokens.resolve_claim(
                payload,
    ...
    async def decode_strict(
        ...
        if cached_cver is None or cached_cver != claim.cver:
            logger.info(f'cver mismatch {cached_cver} {claim.cver} for {claim.sub} {claim.jti}')
    ...
    async def revoke_tokens(
        self,
        access_claim: JwtClaim,
        refresh_token: str
    ) -> None:
        refresh_claim = self.get_token_claim(
            refresh_token,
    ...
    async def refresh_tokens(self, claim: JwtClaim) -> TokenClaim:
        ...
        claim.jti = new_jti
        refresh_token = tokens.encode_jwt_payload(
            self.jwt_policy,
            claim.payload()
        )
