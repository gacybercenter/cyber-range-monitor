...
class UsersService:
    ...
    async def fetch(
        ...
        """
        if rows := await self.users.fetchuser(user_id=user_id, username=username):
            return InternalUser.convert(rows)

        return None

    async def create_user(
        self, body: UserCreateBody, creator_id: uuid.UUID
    ) -> UserSchema:
        """
    ...
    async def patch_by_id(
        self, user_id: uuid.UUID, params: UserPatchBody | UserPatchProfile
    ) -> UserSchema:
        """
        ...
        if params.username and not await self.users.is_username_unique(
            params.username, excluding_id=user_id
        ):
            raise ConflictError('username_taken')
    ...
    async def list_users(self, query: UserQuery, page: PageParams) -> UserPage:
        ...
        results = [
            UserSchema.convert(user) for user in await self.users.list_orms(sql_query)
        ]
