...
class Datasource(db.SqlModel):
    ...
    username: Mapped[str] = mapped_column(
        sql.String(128), nullable=False, doc='Unique name of the datasource'
    )

    password_cipher: Mapped[bytes] = mapped_column(
        sql.BLOB, nullable=False, doc='Encrypted password for the datasource'
    )

    description: Mapped[str | None] = mapped_column(
        sql.String(256), nullable=True, doc='Optional description of the datasource'
    )
    ...
    __table_args__ = (
        sql.CheckConstraint('LENGTH(label) >= 3', name='ck_datasource_label_length'),
    )
...
class Openstack(Datasource):
    ...
    __tablename__ = 'openstack_sources'
...
class Saltstack(Datasource):
    ...
    __tablename__ = 'saltstack_sources'
...
DatasourceType: TypeAlias = Guacamole | Openstack | Saltstack
