...
async def get_guacamole_service(
    ...
    return GuacamoleDatasourceService(
        db=db, crypto_service=crypto_service, guac_tenant=guac_tenant
    )
...
async def get_saltstack_service(
    ...
    return SaltstackService(
        db=db, crypto_service=crypto_service, salt_tenant=saltstack_tenant
    )


async def get_openstack_service(
    db: DatabaseDep, crypto_service: CryptoServiceDep, context: ContextRequired
) -> OpenstackService:
    return OpenstackService(
        db=db, crypto_service=crypto_service, openstack_tenant=context.openstack_tenant
    )
