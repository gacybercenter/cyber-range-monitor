...
async def create_user(
    ...
    '''
    username_taken = await user_service.get_username(create_req.username, db)
    if username_taken:
...
async def delete_user(
    ...
    '''
    await user_service.delete_user(db, user_id, admin.username)
...
async def public_read_all(
    ...
    users = await user_service.role_based_read_all(reader, db)
