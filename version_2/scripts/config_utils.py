...
def config_as_json() -> None:
    ...
    console.print(config.model_dump_json(indent=4))
...
def disable_docs(config: 'CreateConfig', _) -> None:
    keys = [key for key in config.template.keys() if key.lower().startswith('doc')]
...
def export(config: 'CreateConfig', complete_cmd: Optional[str]) -> None:
    ...
    env_file_name = complete_cmd.split(' ', 1)[1] if len(complete_cmd.split(' ', 1)) > 1 else '.env' # type: ignore
...
class CreateConfig:
    ...
    def set_field(self, input_str: str) -> None:
        ...
        if not field_name in self.template:
            self.console.print(f'[red]Unknown property "{field_name}" type help to view properties[/red]')
            return
        
        self.template[field_name] = tokenized[1]
        self.console.print(f'[bold green]Set {field_name}[/bold green] to {value}')
    ...
    def show_self(self, fields_like: Optional[str]) -> None:
        ...
        for key, v in self.template.items():
            if not fields_like or (fields_like and key.lower().startswith(fields_like)):
                self.console.print(f'\t[bold blue]{key}[/bold blue]\t\t[bold green]{v}[/bold green]')
