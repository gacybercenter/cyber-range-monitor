...
def test_index(client, auth):
    ...
    # assert b'by admin on 2018-01-01' in response.data
...
#         db = get_db()
#         db.execute('UPDATE config SET author_id = 2 WHERE id = 1')
#         db.commit()
...
#     with app.app_context():
#         db = get_db()
#         count = db.execute('SELECT COUNT(id) FROM config').fetchone()[0]
#         assert count == 2
